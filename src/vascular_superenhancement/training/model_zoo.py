...
def build_generator(cfg) -> nn.Module:
    ...
    return UNet(
        spatial_dims=3,
        in_channels=cfg.model.in_channels,     # e.g., 3 (vx, vy, vz)
        out_channels=cfg.model.out_channels,   # e.g., 1 (cine prediction)
        channels=cfg.model.channels,           # e.g., [32, 64, 128, 256]
        strides=cfg.model.strides,             # e.g., [2, 2, 2]
        num_res_units=cfg.model.num_res_units  # e.g., 2
    )
...
def build_discriminator(cfg) -> nn.Module:
    ...
    """
    return PatchDiscriminator(in_channels=cfg.model.disc_in_channels)  # e.g., 4
